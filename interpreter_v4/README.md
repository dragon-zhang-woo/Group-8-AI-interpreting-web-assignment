...
- 选择难度：全部、基础、进阶、综合。
- 选择模块或勾选一条 / 多条规则后抽题。
- 单个规则视为专项训练，多个规则视为综合训练；抽题时会优先匹配覆盖更多已选规则的素材。
- 规则树按模块分组，支持全选本组、清空本组、清空全部，并用对勾显示选中规则。
- 输入原文、参考译文和你的译文。
- 点击“诊断”后输出：
  - 诊断：触发了哪些转换规则。
  - 拆解：中文思维、目标语要求、本句应用。
...
- 展示分数趋势和规则触发频次。
- 可按模块和规则筛选。新记录会保存“训练选择规则”和“实际触发规则”，便于区分练习目标和诊断结果。
...
- 选择器采用自定义轻量菜单，保留原生 select 的状态来源，同时用圆角菜单、紧凑字号和对勾显示选中项。
- 左侧规则树采用模块卡片布局，模块标题、说明、规则数和勾选状态分层显示，减少拥挤感。
- 规则地图支持多规则高亮，与左侧规则树使用同一选择状态。
- 素材库位于 `data/materials.json`，当前不少于 120 条，实际为 132 条，覆盖六大模块和七类错误规则。
